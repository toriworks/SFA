...
public class Catalog__10002 extends Activity implements OnClickListener{
...
//		TabLayout = new RelativeLayout( mContext ); 
		...
		for( int i = 0; i < MaxMainTabCount; i++ ){
			...
			if( i == 0 ){
				...
				String ctgryNm = CONTENTS_PAGE_ALL_LIST_CONSTANCE.ARR_CONTENTS_CATEGORY.get( i - 1).ctgryNm;
		...
		layoutContetsView.addView( CA.setAD_GalleryView( strBrandCD, strContentsID, "ALL", strCategoryName) );
	}
	...
	private void setScrollViewTAbBar(){
		...
		layoutContetsView.removeAllViews();
		layoutContetsView.addView( CA.setAD_GalleryView( strBrandCD, strContentsID, "SUB", strCategoryName) );
				...
				for( int i = 0; i < MaxTabbCount; i++){
					if( v == tabImgBtn[i] ){
						...
						if( i == 0 ){
							...
							layoutContetsView.addView( CA.setAD_GalleryView( strBrandCD, CONTENTS_PAGE_ALL_LIST_CONSTANCE.ARR_CONTENTS_CATEGORY_SUB.get( i - 1).ctgryId, "SUB", strCategoryName) );
						}
					}
				}
			}
