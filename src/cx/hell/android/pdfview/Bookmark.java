...
public class Bookmark {
	...
	public ArrayList<BookmarkEntry> getBookmarks(String file) {
		...
		Cursor cur = db.query(true, "bookmark", new String[] { KEY_ENTRY, KEY_COMMENT },
				...
				null, null, null, null);
		Log.d("BookMark", "getBookmarks :" + file);
		if (cur != null) {
			if (cur.moveToFirst()) {
				do {
					list.add(new BookmarkEntry(cur.getString(1), cur.getString(0)));
					Log.d("BookMark", "getBookmarks :" +  cur.getString(0));
	...
	public ArrayList<BookmarkEntry> getBookmarksAll(String file) {
		ArrayList<BookmarkEntry> list = new ArrayList<BookmarkEntry>();
		String md5 = nameToMD5(file);
		...
		Cursor cur = db.query(true, "bookmark", new String[] { KEY_ENTRY, KEY_COMMENT },
				KEY_BOOK + "='" + md5 + "'", null,
				null, null, null, null);
		Log.d("BookMark", "getBookmarksAll :" + file);
		if (cur != null) {
			if (cur.moveToFirst()) {
				do {
					list.add(new BookmarkEntry(cur.getString(1), cur.getString(0)));
	...
	public void deleteBookmark(String file, BookmarkEntry entry) {
		String md5 = nameToMD5(file);
		Log.d("BookMark", "Delete :" + file + " " + entry.toString());		
		int result = db.delete("bookmark", KEY_BOOK + "='" + md5 + "' AND " + KEY_NAME + "= 'user' AND " + KEY_ENTRY + "= ?",
	...
	public void addBookmark(String file, BookmarkEntry entry) {
		...
		cv.put(KEY_TIME, System.currentTimeMillis() / 1000);
		db.insert("bookmark", null, cv);
		Log.d("BookMark", "addBookmark :" + file + " " + entry.toString());	
	}
